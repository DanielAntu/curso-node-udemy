...
module.exports = class PetController {
    ...
    static async removePetById(req,res) {
        ...
        if(pet.user.id.toString() !== user._id.toString()) {
            res.status(422).json({message: 'Houve um problema de solicitação Tente novamente mais tarde!'})
            return
        }
    ...
    static async schedule(req,res) {
        ...
        if (pet.adopter) {
            if(pet.adopter._id.equals(user._id)) {
                res.status(422).json({message: 'Você agendou uma visita para este Pet!'})
                return
            }
        }
